  ...
  struct Walk
  {
                      ...
                      bool metropolis_filter = true)
    {
      ...
      if (metropolis_filter)
      {
        ...
        accumulatedMomentum = prob.mean() * params.momentum * accumulatedMomentum + solver->eta;
        nEffectiveStep = nEffectiveStep + solver->eta * accumulatedMomentum * accept.template cast<NT>();
